...
- **Shape-aware Custom metrics rendering** on the Summary tab — saved queries now render in their natural shape: 1×1 results as bordered tiles (packed 3 per row), multi-row results as inline tables (up to 4 visible rows + `+N more rows` footer), empty results and errors as compact status cards. Replaces the previous one-line teaser that truncated tabular results. Section height is capped at ~50% of the remaining area; overflow is summarized with a `+N metrics not shown` hint
